...
/// S3 INTEGRATED POST ROUTE - Optimized for Instant Upload from profile.ejs
router.post('/profile/edit', upload.single('profile_pic'), (req, res)=> {
    var rules = validationRules.users.update;
    var validator = new asyncValidator(rules);
    
    // 1. Logic: Use new S3 URL if uploaded, otherwise keep the old one from the hidden field
    // req.file.location is the official cloud link generated by your s3Upload middleware
        ...
        if(!errors){
            userModel.updateUser(data, (result)=> {
                if(!result){
                    ...
                    // Success! Redirect back to the profile to see the new S3 image
                    res.redirect('/admin/profile');
                }
            });
        }
        else {
            // HERO FIX: If validation fails during an instant upload from profile.ejs,
            // we render the profile view again with errors instead of the edit view.
            console.log("Validation Errors:", errors);
            res.render('admin/profile', {errs: errors, res: data});
        }
    });
});
